...
class RegisterAccountTestCase(unittest.TestCase):
    ...
    def test_register_valid_account(self):
        ...
        with app.test_client() as client:
            ...
            response = client.post('/delete_account', json=valid_account) # ensure the account does not already exist

            # Test register with valid credentials with no account already existing
            response = client.post('/register', json=valid_account)
    ...
    def test_register_invalid_account(self):
        ...
        with app.test_client() as client:
            ...
            response = client.post('/register', json=invalid_account)
            ...
            response = client.post('/register', json=invalid_json)
            self.assertEqual(response.status_code, 400)
            self.assertIn('error', response.json)
